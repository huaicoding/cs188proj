...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    combos = list()
    for literal_combo in itertools.combinations(literals, 2):
        combos.append(logic.disjoin(~literal_combo[0], ~literal_combo[1]))
    return logic.conjoin(combos)
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    return (findModel(logic.conjoin(KB) & PropSymbolExpr(pacman_str, x1, y1, time=1)),
            findModel(logic.conjoin(KB) & ~PropSymbolExpr(pacman_str, x1, y1, time=1)))
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for coord in non_wall_coords:
            pac_locations.append(PropSymbolExpr(pacman_str, coord[0], coord[1], time=t))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for coord in food:
            curr_food = PropSymbolExpr(food_str, coord[0], coord[1], time=t)
            curr_pac = PropSymbolExpr(pacman_str, coord[0], coord[1], time=t)
            next_food = PropSymbolExpr(food_str, coord[0], coord[1], time=t+1)

            takenFood = curr_pac & curr_food
            missedFood = ~curr_pac & curr_food

            KB.append(takenFood >> ~next_food)
            KB.append(missedFood >> next_food)
    "*** END YOUR CODE HERE ***"
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            cur_loc = PropSymbolExpr(pacman_str, x, y, time=t)
            if findModel(conjoin(KB) & cur_loc):
                possible_locations.append((x, y))

            provable_at = entails(conjoin(KB), cur_loc)
            provable_not_at = entails(conjoin(KB), ~cur_loc)

            if provable_at and provable_not_at:
                print(f'KB entails both A and ~A')

            if provable_at:
                KB.append(cur_loc)
            elif provable_not_at:
                KB.append(~cur_loc)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            ...
            provable_at = entails(conjoin(KB), wall_loc)
            provable_not_at = entails(conjoin(KB), ~wall_loc)
