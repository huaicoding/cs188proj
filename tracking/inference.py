...
def constructBayesNet(gameState: hunters.GameState):
    ...
    pos_range = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    variableDomainsDict[PAC] = pos_range
    variableDomainsDict[GHOST0] = pos_range
    variableDomainsDict[GHOST1] = pos_range

    domain_range = []
    for posx in pos_range:
        for posy in pos_range:
            manh = manhattanDistance(posx, posy)
            for obsv in range(max(0, manh - MAX_NOISE), manh + MAX_NOISE + 1):
                if obsv not in domain_range:
                    domain_range.append(obsv)

    variableDomainsDict[OBS0] = domain_range
    variableDomainsDict[OBS1] = domain_range
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # join all factors by variable
        for joinVariable in eliminationVariables:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, joinVariable)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for key in copyDist:
            cumulativedist[key] = [cumulativeWeight]
            cumulativeWeight += copyDist[key]
            cumulativedist[key].append(cumulativeWeight)

        random_num = random.random()
        for key in copyDist:
            if cumulativedist[key][0] < random_num <= cumulativedist[key][1]:
                return key
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in all_pos:
            if self.beliefs[pos] != 0:
                pos_dist[pos] = self.getPositionDistribution(gameState, pos)

        for pos in all_pos:
            new_prob = 0
            for pos1 in all_pos:
                if self.beliefs[pos1] != 0:
                    new_prob += pos_dist[pos1][pos] * self.beliefs[pos1]
            new_belief[pos] = new_prob
...
class ParticleFilter(InferenceModule):
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        returnedDist = DiscreteDistribution()
        legal = self.legalPositions
        for pos in legal:
            returnedDist[pos] = self.particles.count(pos)

        returnedDist.normalize()
        return returnedDist
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for par in self.particles:
            p = self.getObservationProb(observation, pac_pos, par, jail_pos)
            dist[par] += p
    ...
    def elapseTime(self, gameState):
        ...
        for par in self.particles:
            if par not in dist_map:
                dist = self.getPositionDistribution(gameState, par)
                dist_map[par] = dist
            new_pos = dist_map[par].sample()
